...
public class IntegrateTest {
    ...
    String config = "src/test/resources/web_validation_config.properties";
    String attribute = "code";

    @Before
    public void init() {
        config = FilenameUtils.normalize(config);
        File configFile = org.apache.commons.io.FileUtils.getFile(config);
        if (!configFile.exists()) {
            throw new IllegalStateException(String.format("%s file not found", config));
        }
    }
    ...
    public void TaxInvoice_01() {
        ...
        String XML = FilenameUtils.normalize("src\\test\\resources\\INV71600001.xml");
    ...
    public void TaxInvoice_02() {
        ...
        String XML = FilenameUtils.normalize("src\\test\\resources\\INV71800001.xml");
        JSONObject object = getResponseMessage(XML, config);
        String code = (String) object.get("code");
        showValue("file :" + XML, object.toJSONString());
    ...
    public void TaxInvoice_03() {
        ...
        String XML = FilenameUtils.normalize("src\\test\\resources\\INV71800002.xml");
        JSONObject object = getResponseMessage(XML, config);
        String code = (String) object.get("code");
        showValue("file :" + XML, object.toJSONString());
...
//    	assertEquals(code,"-2");
    ...
    private JSONObject getResponseMessage(String XMPPath, String configPath) {
        ...
        String XML_test = fileUtil.getStringFromFile(FilenameUtils.normalize(XMPPath));
        ValidateController ctrl = new ValidateController(FilenameUtils.normalize(configPath));
